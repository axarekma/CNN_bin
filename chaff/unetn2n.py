...
class DownConv(nn.Module):
    ...
    def forward(self, x):
        if DEBUG:
            print(f'DownConv in {x.shape}')
        if DEBUG:
            print(f'   Expecting {self.in_channels} -> {self.out_channels}')
...
class UpConv(nn.Module):
    ...
    A leaky_relu activation follows each convolution.
    """
    ...
    def forward(self, from_down, from_up):
        ...
        """
        if DEBUG: print(f'Upconv in {from_down.shape} + {from_up.shape}')
        if DEBUG:
            print(
                f'   Expecting {self.in_channels} {self.in_skip} -> {self.out_channels}'
            )
        if DEBUG:
            print(
                f'conv1 {self.in_channels + self.in_skip} -> {self.out_channels}'
            )
            print(f'conv1 {self.out_channels} -> {self.out_channels}')
...
class UNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.fconv1 = conv3x3(self.in_channels + 2 * self.start_filts, 64)
...
if __name__ == "__main__":
    ...
    model = UNet(in_channels=3, depth=2, start_filts=48, merge_mode='concat')
    if DEBUG:
